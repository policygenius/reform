class CompositionTest < ReformSpec
  class SongAndArtist < ReformOneTwoSix::Composition
  ...
  describe "::from" do
    it "creates the same mapping" do
      comp =
      ReformOneTwoSix::Composition.from(
          Class.new(ReformOneTwoSix::Representer) do
  ...
  describe "#nested_hash_for" do
    ...
    it "works with strings in map" do
      Class.new(ReformOneTwoSix::Composition) do
        map(:artist => [["name"]])
