...
module ReformOneTwoSix::Form::Composition
  # Automatically creates a Composition object for you when initializing the form.
  def self.included(base)
    base.class_eval do
      extend ReformOneTwoSix::Form::ActiveModel::ClassMethods # ::model.
      extend ClassMethods
    end
  end

  module ClassMethods
    #include ReformOneTwoSix::Form::ActiveModel::ClassMethods # ::model.

    def model_class # DISCUSS: needed?
      @model_class ||= ReformOneTwoSix::Composition.from(representer_class)
    end
