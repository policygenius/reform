...
require 'reform_one_two_six/representer'
...
module ReformOneTwoSix
  ...
  class Contract # DISCUSS: make class?
    ...
    self.representer_class = ReformOneTwoSix::Representer.for(:form_class => self) # only happens in Contract/Form.
    ...
    module PropertyMethods
      ...
      def properties(*args)
        ...
        if args.first.is_a? Array # TODO: REMOVE in 2.0.
          warn "[ReformOneTwoSix] Deprecation: Please pass a list of names instead of array to ::properties, like `properties :title, :id`."
          args = args.first
        end
    ...
    private
      ...
      def handle_reserved_names(name)
        raise "[ReformOneTwoSix] the property name '#{name}' is reserved, please consider something else using :as." if RESERVED_METHODS.include?(name)
      end
    end
    ...
    require 'reform_one_two_six/contract/setup'
    include Setup
    ...
    require 'reform_one_two_six/contract/validate'
    include Validate
  ...
  private
    ...
    def self.deprecate_as!(options) # TODO: remove me in 2.0.
      ...
      options[:from] = as
      warn "[ReformOneTwoSix] The :as options got renamed to :from. See https://github.com/apotonick/reform/wiki/Migration-Guide and have a nice day."
    end

    def self.deprecate_virtual_and_empty!(options) # TODO: remove me in 2.0.
      if options.delete(:virtual)
        warn "[ReformOneTwoSix] The :virtual option has changed! Check https://github.com/apotonick/reform/wiki/Migration-Guide and have a good day."
        options[:_readable] = true
        options[:_writeable] = false
      end

      if options[:empty]
        warn "[ReformOneTwoSix] The :empty option has changed! Check https://github.com/apotonick/reform/wiki/Migration-Guide and have a good day."
        options[:_readable]  = false
        options[:_writeable] = false
      end
    end
    ...
    require 'reform_one_two_six/schema'
    extend Schema
    ...
    module ReformOneTwoSix20Switch
      def self.included(base)
        base.register_feature(ReformOneTwoSix20Switch)
      end
    end
    def self.reform_2_0!
      include ReformOneTwoSix20Switch
    end
    def self.reform_2_0
      features[ReformOneTwoSix20Switch]
    end
...
require 'reform_one_two_six/contract/errors'
